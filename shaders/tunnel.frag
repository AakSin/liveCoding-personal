...
float map(vec3 p){
    ...
    p += cos(p.zxy*8.)*.125; // Perturb slightly. The mutation gives it a bit more of an organic feel.
       // p -= cos(p.zxy*(sin(time/350.)+1.)*4.)*cos(time/500.); // Perturb slightly. The mutation gives it a bit more of an organic feel.
...
void main( void ){
  ...
  // Camera Setup.
  vec3 camPos = vec3(0, 1, time*2.);  // Camera position, doubling as the ray origin.
